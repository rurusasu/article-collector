...
public AppView の `app.bsky.feed.searchPosts` を使う query-based recommendation source として扱う。この環境では `public.api.bsky.app` が 403 を返すため、unauthenticated search が通る AppView host `api.bsky.app` を使う。
