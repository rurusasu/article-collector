...
`article-collector recommend <SITE_OR_URL>` は `raw.json` に推薦一覧を保存する。`hackernews` / `hn` では Firebase `topstories` からリンク付き story を上位30件取得する。`devto` / `dev.to` では Dev.to トップページのリンクを上から抽出する。その他の `http://` / `https://` URL ではページ内リンクを上から抽出する。
